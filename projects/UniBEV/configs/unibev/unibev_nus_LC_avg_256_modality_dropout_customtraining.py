# End-to-end training for UniBEV_avg
# Average Fusion
...
train_pipeline = [
    ...
    dict(type='LoadMultiViewImageFromFilesWithRandomZeros', to_float32=True), #modified
    #dict(type='PhotoMetricDistortionMultiViewImage'), #removed for now
...
test_pipeline = [
    dict(
        type='LoadPointsFromFileCorruption', #modified
        coord_type='LIDAR',
        load_dim=5,
        use_dim=5
        #test_mode=True #added
    ),
    dict(
        type='LoadPointsFromMultiSweepsCorruption', #modfied
        sweeps_num=10,
        use_dim=[0, 1, 2, 3, 4],
        file_client_args=file_client_args,
        pad_empty_sweeps=True,
...
model = dict(
    type='UniBEV',
    #pretrained='unibev_average_moddrop_trained/unibev_avgfusion_moddrop_256.pth',
    use_grid_mask=True,
...
optimizer = dict(     #FOR CORRUPTIONS!
    type='AdamW',
    lr=5e-4,#lr=1e-3,
    paramwise_cfg=dict(
        custom_keys={
            ...
            'fusion_weight_predictor': dict(lr_mult=1.0),
        }),
    weight_decay=0.0)
# max_norm=10 is better for SECOND
optimizer_config = dict(grad_clip=dict(max_norm=10, norm_type=2))
lr_config = dict(
    policy='fixed',  # Or 'step', or just stick with 'cosine' if you want
    warmup=None
)
#lr_config = dict(
